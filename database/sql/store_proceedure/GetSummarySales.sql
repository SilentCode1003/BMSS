CREATE PROCEDURE `GetSummarySales`(
    IN startdate VARCHAR(20),
    IN enddate VARCHAR(20),
    IN branch VARCHAR(6),
    IN salesstatus VARCHAR(20)
) BEGIN -- insert sales items
...
SELECT CASE
    ...
    CASE
        WHEN SUM(si_quantity * si_price) < 0 THEN 0
        else si_price
    END AS price,
...
WHERE st_date BETWEEN startdate AND enddate
    ...
    AND (
        CASE
            WHEN branch = '0' THEN st_branch IN(
                SELECT mb_branchid
                FROM master_branch
            )
            ELSE st_branch = branch
        END
    )
GROUP BY mp_description,
    dd_description,
    mc_categoryname
ORDER BY total DESC;
...
SELECT dd_name AS item,
    'Discount' AS category,
    COUNT(*) AS quantity,
    0 AS price,
    -- Discounts have no price
